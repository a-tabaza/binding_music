...
class Encoder(L.LightningModule):
    def __init__(self, audio_size, image_size, text_size, graph_size, expansion_factor, contraction_factor, embedding_size):
        ...
        self.image_encoder = nn.Sequential(nn.Linear(image_size, image_size//(contraction_factor*2)), nn.ReLU())
        self.text_encoder = nn.Sequential(nn.Linear(text_size, text_size//(contraction_factor*2)), nn.ReLU())
        self.graph_encoder = nn.Sequential(nn.Linear(graph_size, graph_size//contraction_factor), nn.ReLU())
        self.cat_size = (audio_size//contraction_factor + image_size//(contraction_factor*2) + text_size//(contraction_factor*2) + graph_size//contraction_factor)
